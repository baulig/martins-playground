...
*/
		...
		[Category ("NotOnMac")]
			...
			try {
				...
				Thread.Sleep (2500);
				try {
					int ret = server.Receive (new byte[10], 10, SocketFlags.None);
					Console.WriteLine ("TEST: {0}", ret);
					ret = server.Receive (new byte[10], 10, SocketFlags.None);
					Console.WriteLine ("TEST #1: {0}", ret);
					// server.EndReceive (server.BeginReceive (new byte[10], 0, 10, SocketFlags.None, null, null));
				} catch (Exception ex) {
					Console.WriteLine ("TEST EX: {0}", ex);
				}
